  ...
  <div id="screen-checkout-treez">
    <form class="checkout__form">
      ...
      <div
        ...
        v-bind:class="{ 'checkout-container--qr-mode': aeropayPermalink }"
      >
        <template
          v-if="
            $config.PAYMENT_GATEWAY.NAME === 'Aeropay' &&
              $config.ENABLE_REQUEST_TAXES
          "
        >
          <div
            class="checkout__button aeropay--qr--chekout"
            v-if="aeropayPermalink"
          >
          ...
          <button
            ...
            v-bind:class="{
              'checkout__button--sending': creatingOrder,
          ...
          >
            ...
            <span
              class="checkout__button__background"
              v-bind:class="{ 'checkout__button--left': connected == true }"
            ></span>
          </button>
        </template>
        <button
...
export default {
  ...
  computed: {
    ...
    isName() {
      return (
        this.checkoutType === 'name' ||
        (this.checkoutType !== 'email' && this.checkoutType !== 'phone')
      )
    },
    isPhone() {
  ...
  methods: {
        ...
        .catch(function(error) {
          ...
          switch (error.message) {
            ...
            case 'birthday_not_found_needed':
              mess =
                "We couldn't find you, Please confirm your identity by providing your name and lastname."
              self.showPhoneNumber = false
              self.showBirthday = true
              self.showName = true
              return self.$emit('error', mess)
    ...
    sendOrder: function() {
      ...
      if (self.directCheckoutEnabled) {
        if (this.isEmail) {
          this.notes = self.email
        }
        if (this.isName) {
          this.notes = self.firstname + ' ' + self.lastname
        }
        if (this.isPhone) {
          this.notes = self.phoneNumber
        }
      }
